...
public class Beer : MonoBehaviour, IInteractable
{
    ...
    public bool Interact(Interactor interactor)
    {
        ...
        Time.timeScale = 0;
        option12Text.text = "Admire beers";
        option22Text.text = "Count beers";
        option32Text.text = "Drink beer";
        option12.onClick.AddListener(() =>
        {
            Debug.Log("Admire beers!");
        ...
        option22.onClick.AddListener(() => {
            panel.SetActive(false);
            Debug.Log("Count beers");
            player.DecreaseHunger(3);
            player.IncreaseWellBeing(3);
            timeControllerScript.AddHoursToTime(0.1);

            Debug.Log("dispaly number of beers");
            errorPrompt.gameObject.SetActive(true);
            if (player.beer > 1)
            errorPromptText.text = "There are " + player.beer + " beers!";
            else if (player.beer == 1)
                errorPromptText.text = "There is only 1 beer!";
            else
                errorPromptText.text = "There is no beer :(";
            counter = 500;
            interactionPrompt.gameObject.SetActive(true);
            Time.timeScale = 1;
            RemoveListeners();
