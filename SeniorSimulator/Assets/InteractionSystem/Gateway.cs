...
public class Gateway : MonoBehaviour, IInteractable
{
    ...
    public bool Interact(Interactor interactor)
    {
        ...
        option12.onClick.AddListener(() => { Debug.Log("Go to the Church"); panel.SetActive(false); player.Heal(20); interactionPrompt.gameObject.SetActive(true); });
        option21.onClick.AddListener(() => { Debug.Log("Go to the grocery shop"); panel.SetActive(false); player.TakeDamage(20); interactionPrompt.gameObject.SetActive(true); });
        option22.onClick.AddListener(() => { Debug.Log("Go to the drugstore"); panel.SetActive(false); player.TakeDamage(10); interactionPrompt.gameObject.SetActive(true); });
        option23.onClick.AddListener(() => { Debug.Log("Meet with firends"); panel.SetActive(false); player.TakeDamage(20); interactionPrompt.gameObject.SetActive(true); });
        option32.onClick.AddListener(() => { Debug.Log("Go to the doctor"); panel.SetActive(false); player.Heal(50); interactionPrompt.gameObject.SetActive(true); });
        return true;
    }
}
